...
const username = '15868187925';
const password = '15850537705';
...
const getCookie = async () => {
    ...
    // 等待20秒
    await page.waitForTimeout(20000); // 等待10秒钟
    ...
    // 获取当前页面的所有Cookie
    cookieObj = await page.cookies();

    console.log(cookieObj)

    console.log('CSDN的Cookie信息：', cookieObj);

    const cookie = cookieObj.map(c => `${c.name}=${c.value}`).join('; ');
    console.log(cookie);
...
async function main() {
    ...
    await fetchListByFollow('universsky2015', '')
